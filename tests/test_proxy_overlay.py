...
class StubClient:
    def __init__(self):
        ...
        class Blocks:
            ...
            async def create(self, value: str, label: str, metadata=None):
                block_id = f"block_{len(self._outer.created) + 1}"
                self._outer.created.append((block_id, value, label, metadata))
                return SimpleNamespace(id=block_id)

            async def modify(self, block_id: str, value: str, label: str):
                self._outer.modified.append((block_id, value, label))
                return SimpleNamespace(id=block_id)
...
async def test_overlay_creation_persists_block():
    ...
    overlay_changed, fallback = await manager.apply_overlay("agent-1", "session-1", "system text")
...
async def test_overlay_update_is_idempotent():
    ...
    await manager.apply_overlay("agent-1", "session-1", "system text")
    overlay_changed, fallback = await manager.apply_overlay("agent-1", "session-1", "system text")
...
async def test_overlay_updates_existing_block():
    ...
    await manager.apply_overlay("agent-1", "session-1", "system text")
    overlay_changed, _ = await manager.apply_overlay("agent-1", "session-1", "new guidance")
